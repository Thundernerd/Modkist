...
namespace ModIO.UI
{
    public class ModBrowser : MonoBehaviour
    {
        ...
        private System.Collections.IEnumerator PerformInitialSubscriptionSync()
        {
            ...
            Func<bool> hasUserChanged = () => { return userId != LocalUser.UserId; };
            ...
            if (LocalUser.AuthenticationState == AuthenticationState.ValidToken)
            {
                ...
                requestDelegate = (p) => APIClient.GetUserSubscriptions(userSubFilter,
                    p,
                    (r) =>
                    {
                        request_isDone = true;
            ...
            else
            {
                ...
                requestDelegate = (p) => APIClient.GetAllMods(modFilter,
                    p,
                    (r) =>
                    {
                        request_isDone = true;
            ...
            foreach (ModProfile subProfile in subProfiles)
            {
                bool isBusy = true;
                ...
                APIClient.GetAllModDependencies(subProfile.id,
                    RequestFilter.None,
        ...
        private void FetchAndSetEventIds(Action onComplete = null)
        {
            ...
            idFetchFilter.AddFieldFilter("game_id",
            ...
            if (LocalUser.AuthenticationState == AuthenticationState.ValidToken)
            {
                APIClient.GetUserEvents(idFetchFilter,
                    pagination,
                    (r) =>
                    {
                        if (r.items.Length > 0)
            ...
            // fetch mod event id
            APIClient.GetAllModEvents(idFetchFilter,
                pagination,
                (r) =>
                {
                    if (r.items.Length > 0)
        ...
        private System.Collections.IEnumerator FetchUserRatings()
        {
            ...
            if (LocalUser.AuthenticationState == AuthenticationState.ValidToken && !isRequestDone)
            {
                ...
                APIClient.GetUserRatings(filter,
                    pagination,
                    (r) =>
                    {
                        response = r;
        ...
        protected System.Collections.IEnumerator ProcessModUpdates(List<ModEvent> modEvents)
        {
            ...
            if (modEvents != null && modEvents.Count > 0)
            {
                ...
                if (modfileChanged.Count > 0)
                {
                    ...
                    // send request
                    APIClient.GetAllMods(modFilter,
                        pagination,
                        (r) =>
                        {
                            isRequestDone = true;
        ...
        public void OnSubscribedToMod(int modId)
        {
            ...
            APIClient.GetAllModDependencies(modId,
                ...
                page =>
                {
                    foreach (ModDependency modDependency in page.items)
                    {
                        if (!LocalUser.SubscribedModIds.Contains(modDependency.modId) &&
                            !LocalUser.QueuedSubscribes.Contains(modDependency.modId))
                        {
                            SubscribeToMod(modDependency.modId);
                        }
                    }
        ...
        private void UpdateSubscriptionReceivers(
            IList<int> addedSubscriptions,
            IList<int> removedSubscriptions
        )
        {
        ...
        // ---------[ OBSOLETE ]---------
        [Obsolete("No longer used.")] public const string MANIFEST_FILENAME = "browser_manifest.data";
        ...
        [Obsolete] [HideInInspector] public InspectorView inspectorView;
        [Obsolete] [HideInInspector] public SubscriptionsView subscriptionsView;
        [Obsolete] [HideInInspector] public LoginDialog loginDialog;
        [Obsolete] [HideInInspector] public UserView loggedUserView;

        [Obsolete("Use AuthenticatedUserViewController.m_guestData instead.")]
        private UserDisplayData m_guestData;

        [Obsolete] private UserProfile m_userProfile;
        ...
        public void RequestExplorerPage(
            int pageIndex,
